...
async function renderAIMemo(){
  ...
  const financialSummary=`Company: ${settings.company} (${settings.industry})\nPeriod: ${period}\nTotal expenses: ${sym}${fmt(totalExp)}\n${priorSummary}\nTop expense categories:\n${topCats.map(([c,v])=>`  ${c}: ${sym}${fmt(v)} (${totalExp>0?(v/totalExp*100).toFixed(1):0}% of total)`).join('\n')}\nNumber of flagged transactions: ${flagged.length}\nFlag types present: ${[...new Set(flagged.map(r=>r.flag.includes('50%')?'M&E 50% rule':r.flag.includes('T4A')||r.flag.includes('1099')?'T4A/1099 threshold':r.flag.includes('capital')||r.flag.includes('capitalize')?'Capitalize vs expense':r.flag.includes('duplicate')||r.flag.includes('reversal')?'Duplicate/reversal':r.flag.includes('SEVERANCE')?'Severance disclosure':'Other')).join(', ')||'None'}\nTrial balance: ${balanced?'BALANCED':'NOT BALANCED — difference of '+sym+fmt(Math.abs(totDr-totCr))}`.trim();
